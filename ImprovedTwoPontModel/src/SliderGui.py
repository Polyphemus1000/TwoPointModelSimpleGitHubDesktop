...
class SliderGui():
    ...
    def fmomentumLoss(self, x, Ttarget = 5, n=1.5):
           Tnorm = x/Ttarget
           if Tnorm >= 1:
               return 1
           else:
    ...
    def calculate(self, pp,gg, dd,ee,ff):
        ...
        try:
            ...
            # print(NumberDensitySOLMid)
             P.Calculate(NumberDensitySOLMid, self.ParallelHeatFlux, fmol, fmomentum, fconduction) # use the calculate function present in all solvers
             ...
             self.NumberDensityAtTarget = P.NumberDensityTarget #make the results available for the rest of the program
        ...
        for r in np.arange(1,12,1):
            for q in np.arange(1,12,1):
                for n in np.arange(1,12,1):
                    for m in np.arange(1, 10, 0.1):
                        #print('This is t',m)
                        self.calculate(p*m, g, n/10, q/2, r/10)
